...
parser.add_argument("--fssoc", "--fss", "--osoc", "--onesoc", action="store_true", help="Plot the result of the one-shot SOC.")
...
#figs = 12
#fs = 50 ; fsm = 50
cmap = args.cm if args.cm else 'inferno' 
cmapcolor = plt.cm.inferno  # cmapcolor = plt.cm.nipy_spectral
if cmap == "jet2" :
...
class klinedecomp :
	def __init__( self, ax , bcomp , lev , cmapcolor ) :
		...
		if basis.find("j")>-1 or basis.find("z")>-1 :
			...
			self.Z = np.array( Z[bcomp+ni*nbasis] ,dtype=float )
			print "[", bcomp+ni*nbasis, "component]"
...
for dd in range(ndir) : 
	...
	if args.fs :
		...
		dispfaxis 	 = "naxis{}_Nplot{}_Nk{}_{}{:d}th{}.dx".format( naxis, nplot, nktot, tagEpBr, hobj.count, anglelab) 
		dispfaxisdecomp  = "decomp_naxis{}_Nplot{}_Nk{}_{}{:d}th{}.dx".format( naxis, nplot, nktot, tagEpBr, hobj.count, anglelab ) 
		...
		faxis  = "naxis{}_Nplot{}_Nk{}_{}{:d}th{}.dx".format( naxis, nplot, nktot, tagEpBr, count, anglelab) 
		faxisdecomp  = "decomp_naxis{}_Nplot{}_Nk{}_{}{:d}th{}.dx".format( naxis, nplot, nktot, tagEpBr, count, anglelab) 
		fspect		= filehead + fspect
		fspectdecomp	= filehead + fspectdecomp
		fspect		= cwd + fspect
		fspectdecomp	= cwd + fspectdecomp
		...
		if args.decomp: 
			...
			try : 
				...
				faxis  = cwd + "{}_naxis{}_Nplot1_Nk40401_{}{:d}th{}.dx".format( fileind, naxis, tagEpBr, count, anglelab ) 
				faxisdecomp  = cwd + "decomp_{}_naxis{}_Nplot1_Nk40401_{}{:d}th{}.dx".format( fileind, naxis, tagEpBr, count, anglelab ) 
				fsurface( ax[1][0] ) ;
			except : 
				print "Warnning in"
				print fspect 
...
if args.fs :  
	...
	if args.decompall :
		...
		wspace =( 0.00 if (args.wspace  is None) else float(args.wspace ) )
		hspace =( 0.00 if (args.hspace  is None) else float(args.hspace ) )
		if args.ldos : 
